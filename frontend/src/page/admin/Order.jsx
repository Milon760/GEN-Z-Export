      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-zinc-400 mt-0.5">
            Total Orders: {allOrder?.length || 0}
          </p>
        </div>
      </div>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-zinc-800 text-sm font-medium text-slate-700 dark:text-zinc-300">
                  {allOrder.map((order) => (
                    <tr
                      key={order._id}
                      className="hover:bg-slate-50/60 dark:hover:bg-zinc-800/30 transition-colors duration-200"
                    >
                      ...
                      <td
                        className="px-5 py-4 max-w-[200px] truncate text-slate-600 dark:text-zinc-400"
                      ...
                      <td className="px-5 py-4 whitespace-nowrap">
                        <span
                          className={`px-2.5 py-1 rounded-full text-xs font-semibold border ${getStatusStyle(order.orderStatus)}`}
                        >
                          {order.orderStatus}
                        </span>
                      </td>
              ...
              >
                ...
                <div className="flex justify-between items-center border-b border-slate-100 dark:border-zinc-800 pb-2.5">
                  <span className="text-xs font-mono font-bold text-slate-400 dark:text-zinc-500">
                    ID: {order.userId?.substring(0, 10)}...
                  </span>
                  <span
                    className={`px-2.5 py-0.5 rounded-full text-xs font-semibold border ${getStatusStyle(order.orderStatus)}`}
                  >
                    {order.orderStatus}
                  </span>
                </div>
