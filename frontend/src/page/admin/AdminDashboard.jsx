...
const AdminDashboard = () => {
  const { allUser, allProduct, allOrder, userPagination } = useAdmin();
  console.log("alllll", allUser);

  // Total revenue dynamic calculation setup
  const totalRevenue =
    allOrder?.reduce((acc, order) => acc + (order.totalAmount || 0), 0) ||
    125400;
  ...
  const stats = [
    {
      title: "Total Users",
      value: userPagination.totalUser,
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight flex items-center gap-2">
            <FiGrid className="text-blue-500 animate-pulse" /> Overview
            Dashboard
          </h1>
        ...
        {stats.map((stat, idx) => (
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <div
                className={`p-2.5 rounded-xl transition-transform duration-300 group-hover:scale-110 ${stat.bgLight} ${stat.bgDark}`}
              >
                {stat.icon}
              </div>
            </div>
        ...
        <div className="lg:col-span-2 p-6 rounded-2xl border bg-white border-slate-200 dark:bg-zinc-900 dark:border-zinc-800">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold flex items-center gap-2">
              <FiShoppingBag className="text-violet-500" /> Recent Orders
              Management
            </h2>
                    ...
                    >
                      ...
                      <td className="py-3.5 text-right">
                        ...
                        <div className="flex items-center justify-end gap-2 opacity-80 group-hover:opacity-100 transition-opacity">
                          <button
                            title="View Details"
                            className="p-1 rounded hover:bg-slate-200 dark:hover:bg-zinc-700 text-slate-600 dark:text-zinc-400"
                          >
          ...
          <div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-zinc-800/40 border border-slate-100 dark:border-zinc-800">
                ...
                <span className="text-sm font-bold bg-white dark:bg-zinc-900 px-2.5 py-1 rounded-lg border border-slate-200 dark:border-zinc-800 shadow-sm">
                  {allProduct?.length || 0}
                </span>
              </div>
              <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-zinc-800/40 border border-slate-100 dark:border-zinc-800">
