  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result?.success) {
        ...
        throw new Error(
          result?.message || "Something went wrong while creating the product.",
        );
      }
    } catch (error) {
      setStatusMessage({
  ...
  return (
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-900 flex items-center justify-center p-4 sm:p-6 lg:p-8 transition-colors duration-500 antialiased font-sans select-none">
      <div className="w-full max-w-4xl bg-white dark:bg-neutral-950 rounded-[2rem] shadow-[0_30px_70px_rgba(0,0,0,0.03)] dark:shadow-[0_30px_70px_rgba(0,0,0,0.3)] overflow-hidden border border-neutral-200/60 dark:border-neutral-800/60 transition-all duration-500 backdrop-blur-sm">
        ...
        <div className="relative bg-neutral-950 dark:bg-neutral-950 border-b border-neutral-800 p-6 sm:p-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4 group">
          ...
          <div className="relative z-10 text-left space-y-1">
            <h2 className="text-xl sm:text-2xl font-black text-white tracking-tight flex items-center gap-2 uppercase">
              <Package className="w-5 h-5 sm:w-6 h-6 text-[#C5A059]" /> HQ Core:
              Deploy Drop
            </h2>
            <p className="text-neutral-400 text-xs sm:text-sm font-medium max-w-xl">
              Populate the structural attributes mapping pipeline below to
        ...
        {statusMessage.type && (
          <div
            className={`mx-6 sm:mx-10 mt-6 p-4 rounded-2xl flex items-start gap-3 border transition-all duration-300 transform scale-100 ${
          ...
          >
            ...
            <div className="text-xs sm:text-sm font-bold tracking-wide text-left">
              {statusMessage.text}
            </div>
            <button
              type="button"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 lg:gap-6">
            ...
            <div className="text-left space-y-1.5">
              <label className="text-xs font-black uppercase tracking-wider text-neutral-600 dark:text-neutral-400 flex items-center gap-1.5">
                <Tag className="w-3.5 h-3.5 text-neutral-400" /> Identifier SKU{" "}
            ...
            <div className="text-left space-y-1.5">
              <label className="text-xs font-black uppercase tracking-wider text-neutral-600 dark:text-neutral-400 flex items-center gap-1.5">
                <Package className="w-3.5 h-3.5 text-neutral-400" /> Drop Title{" "}
                <span className="text-red-500">*</span>
              </label>
              <input
                type="text"
            ...
            <div className="text-left space-y-1.5">
              <label className="text-xs font-black uppercase tracking-wider text-neutral-600 dark:text-neutral-400 flex items-center gap-1.5">
                <Layers className="w-3.5 h-3.5 text-neutral-400" /> Cluster
            ...
            <div className="text-left space-y-1.5">
              <label className="text-xs font-black uppercase tracking-wider text-neutral-600 dark:text-neutral-400 flex items-center gap-1.5">
                <CircleDollarSign className="w-3.5 h-3.5 text-neutral-400" />{" "}
                Active Price (৳) <span className="text-red-500">*</span>
              </label>
              <input
                type="number"
                name="price"
            ...
            <div className="text-left space-y-1.5">
              <label className="text-xs font-black uppercase tracking-wider text-neutral-600 dark:text-neutral-400 flex items-center gap-1.5">
                <CircleDollarSign className="w-3.5 h-3.5 text-neutral-400" />{" "}
                Ceiling Price (৳) <span className="text-red-500">*</span>
              </label>
              <input
                type="number"
                name="original_price"
            ...
            <div className="text-left space-y-1.5">
              <label className="text-xs font-black uppercase tracking-wider text-neutral-600 dark:text-neutral-400 flex items-center gap-1.5">
                <Layers className="w-3.5 h-3.5 text-neutral-400" /> Stock
            ...
            <div className="text-left space-y-1">
              <label className="text-xs font-black uppercase tracking-wider text-neutral-600 dark:text-neutral-400 flex items-center gap-1.5">
                <Palette className="w-3.5 h-3.5 text-neutral-400" /> Aesthetic
                Hues <span className="text-red-500">*</span>
              </label>
              <span className="text-[10px] text-neutral-400 font-medium block">
                Split nodes using comma separators
              </span>
              <input
                type="text"
                name="colors"
            ...
            <div className="text-left space-y-1">
              <label className="text-xs font-black uppercase tracking-wider text-neutral-600 dark:text-neutral-400 flex items-center gap-1.5">
                <Maximize className="w-3.5 h-3.5 text-neutral-400" /> Dimension
                Scaling <span className="text-red-500">*</span>
              </label>
              <span className="text-[10px] text-neutral-400 font-medium block">
                Split nodes using comma separators
              </span>
            ...
            <div className="md:col-span-2 text-left space-y-1.5">
              <label className="text-xs font-black uppercase tracking-wider text-neutral-600 dark:text-neutral-400 flex items-center gap-1.5">
                ...
                Metric Ranking Verification (0.0 to 5.0){" "}
                <span className="text-red-500">*</span>
              </label>
              <input
                type="number"
            ...
            <div className="md:col-span-2 text-left space-y-2">
              <label className="text-xs font-black uppercase tracking-wider text-neutral-600 dark:text-neutral-400">
                Integrated Image Manifest{" "}
                <span className="text-red-500">*</span>
              </label>

              <div
                className={`relative border-2 border-dashed rounded-2xl p-6 transition-all duration-500 flex flex-col items-center justify-center min-h-[160px] transform ${
                  preview
                ...
                {!preview ? (
                  <>
                    ...
                    <div className="text-center space-y-0.5">
                      <p className="text-neutral-950 dark:text-white font-black text-xs uppercase tracking-wider">
                        Mount New Media File
                      </p>
                      <p className="text-neutral-400 text-[10px] font-medium">
                        Supports High-res PNG, JPG, WEBP formats (Max 3MB)
                      </p>
                    </div>
                  </>
                ) : (
                  <div className="w-full flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                      <div className="w-20 h-20 rounded-xl overflow-hidden border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-1 shrink-0 shadow-md">
                        <img
                          src={preview}
                          alt="Upload drop preview"
                          className="w-full h-full object-cover rounded-lg"
                        />
                      </div>
                      <div className="space-y-1">
                        <p className="text-neutral-900 dark:text-white font-bold text-xs truncate max-w-[180px] sm:max-w-xs">
                          {image?.name}
                        </p>
                        <p className="text-neutral-400 text-[10px] font-bold">
                          {(image?.size / (1024 * 1024)).toFixed(2)} MB
                        </p>
                        <span className="inline-flex items-center px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-wider bg-emerald-500/10 text-emerald-500">
                          Staged
                        </span>
                      </div>
                    </div>
            ...
            <div className="md:col-span-2 text-left space-y-1.5">
              <label className="text-xs font-black uppercase tracking-wider text-neutral-600 dark:text-neutral-400 flex items-center gap-1.5">
                <FileText className="w-3.5 h-3.5 text-neutral-400" /> Elaborate
