...
const Verify = () => {
  ...
  const [message, setMessage] = useState(
    "Synchronizing token security parameters...",
  );
  ...
  const fetchUser = useCallback(
    async (isManualRetry = false) => {
      ...
      if (!token) {
        setStatus("error");
        setMessage(
          "Verification security token node missing from url parameters.",
        );
        return;
      }
      ...
      try {
        ...
        if (response && response.success) {
          ...
          setMessage(
            response.message ||
              "Identity established successfully! Routing access terminal...",
          );
          ...
          setMessage(
            response?.message ||
              "Token synchronization rejected or expired link.",
          );
        }
      } catch (error) {
        ...
        setStatus("error");
        setMessage(
          "Network layer disruption or authentication database core offline.",
        );
      }
    },
  ...
  return (
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950 text-neutral-900 dark:text-neutral-100 font-sans antialiased select-none flex items-center justify-center p-4 sm:p-6 lg:p-8 relative overflow-hidden transition-colors duration-700">
      ...
      <div
        className="absolute top-1/2 left-1/2 w-[500px] h-[500px] bg-[#C5A059]/10 rounded-full blur-[140px] pointer-events-none transform -translate-x-1/2 -translate-y-1/2 animate-pulse"
      ...
      <div
        className={`w-full max-w-md bg-white/80 dark:bg-neutral-900/60 backdrop-blur-xl rounded-[2.5rem] shadow-[0_30px_70px_rgba(0,0,0,0.03)] dark:shadow-[0_4px_40px_rgba(0,0,0,0.4)] border p-8 sm:p-12 text-center transition-all duration-500 relative z-10 group ${
          status === "success"
      ...
      >
        ...
        <div className="relative w-28 h-28 mx-auto mb-8 flex items-center justify-center">
          ...
          <div
            className={`absolute inset-0 border border-dashed rounded-full scale-100 transition-all duration-700 ${
              status === "success"
                ? "border-emerald-500/50 animate-spin"
                : status === "error"
                  ? "border-rose-500/50 animate-none"
                  : "border-[#C5A059]/40 animate-spin"
            }`}
        ...
        <div className="space-y-3 mb-8">
          ...
          <p
            className={`text-xs sm:text-sm font-semibold px-4 leading-relaxed min-h-[40px] transition-all duration-300 ${
              status === "success"
        ...
        {status === "error" && (
          <div className="pt-2">
            ...
            >
              <FiRefreshCw
                size={13}
                className={`${isLoading ? "animate-spin" : "group-hover/btn:rotate-180"} transition-transform duration-500`}
              />
